...
var login = {
    ...
    check_mobile_number_exist: function() {
        var mobileNumber = $('#mobile_number').val().trim(); // Trim any leading/trailing spaces
    
        if (!mobileNumber) {
            toastr.error('Please enter a mobile number.', 'Error');
            return; // Exit the function if the mobile number is empty
        }
    
        var obj = { mobile_number: mobileNumber };
        ...
        $.ajax({
            ...
            success: function (responce) {
                if (responce.title == "success") {
                    ...
                    location.href = login.base_url + "/admin-login";
