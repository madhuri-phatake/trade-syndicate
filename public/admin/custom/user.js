var user = {
    ...
    bind_events: function () {
        var self = this;
        $('.access-controll').prop("disabled", !self.edit); // Corrected line

        $("#user_access").on('click', function () { // Corrected event handler
        ...
        $("#create_user").on('click', function (event) {
            ...
            var form = $('form[id="user_form"]');
            if (form.valid()) { // Validate the form
                user.add_user();
            }
    ...
    get_users: function () {
        ...
        if (obj.user_access && obj.user_access.length > 0) {
